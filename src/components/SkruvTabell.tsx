...
import { skruv } from "@/lib/data";
import { normalize } from "@/lib/search";
import { withBase } from "@/lib/basepath";
import CopyButton from "./CopyButton";
import { SearchIcon, CloseIcon } from "./Icons";

export default function SkruvTabell() {
  const [filter, setFilter] = useState("");

  const filtrerade = useMemo(() => {
    const q = normalize(filter);
    if (!q) return skruv;
    return skruv.filter((s) => {
    ...
    <div>
      <div className="mb-4 flex items-center gap-2 rounded-2xl border border-border bg-card px-4 py-1 shadow-sm transition-colors focus-within:border-brand sm:max-w-sm">
        ...
        <input
          ...
          aria-label="Filtrera skruvlängder"
          placeholder="Sök t.ex. 3 tum eller 75…"
      ...
      {filtrerade.length === 0 ? (
        <p className="text-ink-soft">Ingen skruv matchar ”{filter}”.</p>
      ) : (
        <div className="card overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-left">
              <caption className="sr-only">
                Traditionella träskruvslängder i tum och vad de motsvarar i millimeter
              </caption>
              <thead>
                <tr className="border-b border-border bg-card-muted text-xs font-bold uppercase tracking-wide text-muted">
                  ...
                  <th scope="col" className="px-4 py-3">Längd</th>
                  ...
                  >
                    <th scope="row" className="px-4 py-3 font-extrabold text-ink">
                      <span className="flex items-center gap-3">
                        <span className="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-border bg-white">
                          <Image
                            src={withBase(s.bild)}
                            alt=""
                            width={72}
                    ...
                    <td className="px-4 py-3 text-right">
                      <CopyButton value={`${s.tum} träskruv = ${s.mm} mm`} compact />
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <p className="border-t border-border bg-card-muted px-4 py-3 text-sm text-muted">
            Skruv följer samma tumlogik som spik: en tretumsskruv är 75 mm. Tänk på att skruvens
            längd väljs så att minst halva skruven fäster i det bakre virket.
          </p>
        </div>
      )}
    </div>
